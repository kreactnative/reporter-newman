...
module.exports = function (emitter, options, collectionRunOptions) {
    ...
    emitter.on('start', err => {
        ...
        let description = options.description || 'Newman launch';
    ...
    function sendResponseLogs(stepId, response) {
        ...
        var responseBody=new StringDecoder('utf8').write(response.stream);
        if(responseBody.length>65535)
            responseBody=responseBody.substring(0, 65534);

        logMessage(stepId, `Response: Body: ${responseBody}`);
    }
